...
/**
 * InventoryModal — RPG inventory / equipment modal.
 *
 * Design reference recreated as a self-contained React component.
 * Styling is inline (no external CSS) so it drops into any codebase; port to
 * your styling solution (CSS modules / Tailwind / styled-components) as needed.
 *
 * Requires the Google fonts "Cinzel" and "Nunito Sans" to be loaded by the host
 * app, and the image assets in ./assets (see README).
 *
 * Props:
 *   items         Array<Item>   inventory items (see shape below). Defaults to demo data.
 *   character     Character     { name, level, cls, avatar }
 *   currencies    Array<Currency>
 *   equipment     { left: Slot[], right: Slot[] }  paperdoll slot layout
 *   stats         Array<Stat>
 *   showBackdrop  boolean       blurred map behind the modal (default true)
 *   showSearch    boolean       (default true)
 *   showStats     boolean       (default true)
 *   assetBase     string        prefix for bundled asset paths (default "assets/")
 *   onClose       () => void
 *   onPrimary     (item) => void   Equip / Use / Sell
 *   onDrop        (item) => void
 *
 * Item:      { id, name, cat, rarity, qty, glyph?, img?, value, desc }
 *   cat:     "equipement" | "consommables" | "ressources" | "grimoires"
 *   rarity:  "common" | "uncommon" | "rare" | "epic" | "legendary"
 *   glyph:   key into GLYPHS (used when img is absent)
 *   img:     asset path (overrides glyph)
 * Slot:      { label, itemId | null, empty }   empty = glyph key for empty slot
 * Stat:      { label, value, glyph, color }
 * Currency:  { icon, value, color }
 */
...
const GOLD = "#e3b64f";
const GOLD_LIGHT = "#f2d488";

/* ------------------------------- svg glyphs ------------------------------ */
const GLYPHS = {
  infinity: '<path d="M6 8a4 4 0 100 8c3 0 5-8 8-8a4 4 0 110 8c-3 0-5-8-8-8z"/>',
  sword: '<path d="M14.5 4.5L20 4l-.5 5.5-8 8"/><path d="M4 20l3.5-3.5"/><path d="M6.5 13.5l4 4"/><path d="M4 17l3 3"/>',
  potion: '<path d="M9 3h6"/><path d="M10 3v4l-3.5 8A3.5 3.5 0 0010 20h4a3.5 3.5 0 003.5-5L14 7V3"/><path d="M7.5 14h9"/>',
  material: '<path d="M4 8a2.5 4 0 004 0 2.5 4 0 00-4 0z"/><path d="M6 4h9a4 4 0 010 8H6"/><path d="M6 12h9a4 4 0 010 8H6"/>',
  ...
  scroll: '<path d="M6 5a2 2 0 012-2h9v14a2 2 0 002 2H8a2 2 0 01-2-2z"/><path d="M17 3a2 2 0 012 2v0a2 2 0 01-2 2"/><path d="M9 8h5M9 12h5"/>',
  ...
  feather: '<path d="M20 4c-9 1-13 7-14 14"/><path d="M6 18C4 12 8 6 20 4c0 6-3 12-9 14z"/><path d="M11 13l4-4"/>',
  snail: '<circle cx="14" cy="14" r="5.5"/><path d="M14 14a2 2 0 112.5-2"/><path d="M8.5 19.5H4M4 12l3-2M5 10l1-3"/>',
...
function ItemIcon({ item, color, size = "80%", assetBase }) {
  if (item.img) {
    return (
      <img
        src={assetBase + item.img.replace(/^assets\//, "")}
        alt={item.name}
        style={{
          width: size,
          height: size,
  ...
  return <Glyph name={item.glyph} color={color} size={size} />;
}
...
const DEMO_EQUIPMENT = {
  left: [
    { label: "Tête", itemId: 7, empty: "helmet" },
    { label: "Amulette", itemId: 9, empty: "amulet" },
    { label: "Torse", itemId: 10, empty: "chest" },
    { label: "Cape", itemId: null, empty: "cape" },
    { label: "Mains", itemId: null, empty: "gloves" },
  ],
  right: [
    { label: "Arme", itemId: 1, empty: "sword" },
    { label: "Bouclier", itemId: null, empty: "shield" },
    { label: "Anneau", itemId: 11, empty: "ring" },
    { label: "Ceinture", itemId: null, empty: "belt" },
    { label: "Bottes", itemId: 12, empty: "boots" },
  ],
};
...
const DEMO_CHARACTER = { name: "neraën", level: 20, cls: "Aventurière", avatar: "avatar.png" };
...
export default function InventoryModal({
  ...
  const asset = (f) => assetBase + f.replace(/^assets\//, "");
  ...
  const primaryLabel =
    selected.cat === "consommables" ? "Utiliser" : selected.cat === "equipement" ? "Équiper" : "Vendre";

  const renderSlot = (def, key) => {
    const it = def.itemId ? byId(def.itemId) : null;
    ...
    return (
      <button
        ...
        onMouseLeave={(e) =>
          (e.currentTarget.style.borderColor = isSel
            ? GOLD
            : it
            ? RARITY[it.rarity]
            : "rgba(159,195,201,.28)")
        ...
        style={{
          ...
          border: `2px solid ${isSel ? GOLD : it ? RARITY[it.rarity] : "rgba(159,195,201,.28)"}`,
          boxShadow: isSel ? "0 0 0 3px rgba(227,182,79,.5)" : "none",
          display: "flex",
          alignItems: "center",
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            >
              <Glyph name="material" color={GOLD} size="24px" />
            </div>
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
            {currencies.map((c, i) => (
              ...
              >
                ...
                />
                <span style={{ fontWeight: 800, fontSize: 15, color: c.color }}>{c.value}</span>
              </div>
            ))}
            <button
              onClick={onClose}
        ...
        <div style={{ flex: 1, display: "flex", gap: 20, padding: 20, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            >
              {TABS.map((t) => {
                ...
                const count =
                  t.id === "tous" ? items.length : items.filter((x) => x.cat === t.id).length;
                return (
                  <button
                    key={t.id}
                    onClick={() => setTab(t.id)}
                    onMouseEnter={(e) => {
                      if (!active) e.currentTarget.style.background = "rgba(227,182,79,.12)";
                    }}
                    onMouseLeave={(e) => {
                  ...
                  >
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, color: "#6f959b" }}>{count}</span>
                  </button>
                );
              })}
            </div>
            ...
            {showSearch && (
              ...
              >
                <span style={{ display: "flex", color: "#6f959b" }}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border: `2.5px solid ${isSel ? GOLD : RARITY[it.rarity]}`,
                        boxShadow: isSel
                          ? "0 0 0 3px rgba(227,182,79,.55), 0 8px 22px rgba(0,0,0,.45)"
                          : "0 3px 8px rgba(0,0,0,.28)",
                        display: "flex",
                    ...
                    >
                      <span
                        style={{
                          ...
                          height: 10,
                          borderRadius: "50%",
                          background: isSel ? GOLD : RARITY[it.rarity],
                        }}
                      />
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ width: 6, height: 20, borderRadius: 3, background: GOLD }} />
              <div
                style={{
                  fontFamily: "'Cinzel', serif",
                  fontWeight: 700,
              ...
              <div
                style={{ marginLeft: "auto", fontSize: 13, fontWeight: 700, color: "#9fc3c9" }}
              >
            ...
            >
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {equipment.left.map((e, i) => renderSlot(e, "l" + i))}
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {equipment.right.map((e, i) => renderSlot(e, "r" + i))}
              </div>
            </div>
